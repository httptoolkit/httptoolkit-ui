...
const simplifyHarForSnippetExport = (harRequest: ExtendedHarRequest) => {
    ...
    // this, we drop headers that tools can produce automatically for themselves. The actual
    // header-filtering rules live in `snippet-export-sanitization.ts` so the worker and the
    // UI share one source of truth (see review point #7).
