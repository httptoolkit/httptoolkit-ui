...
const isSendRequestCard = (
  key: SendCardKey
): key is "requestHeaders" | "requestBody" => key.startsWith("request");

const isSentResponseCard = (
  key: SendCardKey
): key is "responseHeaders" | "responseBody" => key.startsWith("response");
...
type ExpandableSendRequestCardKey =
  (typeof EXPANDABLE_SEND_REQUEST_CARD_KEYS)[number];

const EXPANDABLE_SENT_RESPONSE_CARD_KEYS = [
  "responseHeaders",
  "responseBody",
] as const;
type ExpandableSentResponseCardKey =
  (typeof EXPANDABLE_SENT_RESPONSE_CARD_KEYS)[number];

type ExpandableSendCardKey =
  | ExpandableSendRequestCardKey
  | ExpandableSentResponseCardKey;
...
export class UiStore {
  ...
  buildCustomTheme(themeFile: string) {
    ...
    if (
      ...
      throw new Error(
        "Theme must contain an `extends` field with a built-in theme name (dark/light/high-contrast)"
      );
    }
  ...
  get viewCardProps() {
    return _.mapValues(this.viewCardStates, (state, key) => ({
      ...
      expanded:
        key === this.animatedExpansionCard
          ? ("starting" as const)
  ...
  get sendCardProps() {
    return _.mapValues(this.sendCardStates, (state, key) => {
      const expandedState =
        key === this.expandedSendRequestCard ||
      ...
      return {
        ...
        onCollapseToggled: this.toggleSendCardCollapsed.bind(
          this,
          key as SendCardKey
        ),
  ...
  toggleSendCardCollapsed(key: SendCardKey) {
    ...
    if (siblingCards.every((k) => this.sendCardStates[k].collapsed)) {
      ...
      const bestAlternativeCard =
        keyIndex === siblingCards.length - 1
  ...
  @observable
  searchFilters: FilterSet = emptyFilterSet();

  @action.bound
  setSearchFiltersValue(value: FilterSet) {
