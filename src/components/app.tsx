...
const Route = ({
  ...
  return <props.pageComponent {...otherProps}>{children}</props.pageComponent>;
};
...
class App extends React.Component<{ accountStore: AccountStore }> {
  ...
  get canVisitSettings() {
    return (
      ...
      this.props.accountStore.isPastDueUser ||
      true
    );
  }
  ...
  componentDidMount() {
    disposeOnUnmount(
      this,
      appHistory.listen(({ location }) => trackPage(location))
    );
  }
  ...
  render() {
    ...
    return (
      <LocationProvider history={appHistory}>
        ...
        {modal === "post-checkout" && (
          <CheckoutSpinner onCancel={cancelCheckout} />
        )}
      </LocationProvider>
    );
  }
...
const AppWithStoreInjected = App as unknown as WithInjected<
  typeof App,
  "accountStore"
>;
