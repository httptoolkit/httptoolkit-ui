...
const RowContainer = styled(LittleCard)<{
  ...
  &:focus {
    outline: none;
    box-shadow: 0 0 1px 2px ${(p) => polished.rgba(p.theme.popColor, 0.5)};
    background-color: ${(p) => p.theme.mainBackground};
  }
`;

export const AddRuleRow = styled(
  (
    p: {
      onAdd: () => void;
      disabled: boolean;
  ...
  background-color: ${(p) => polished.rgba(p.theme.mainBackground, 0.4)};
...
export class RuleRow extends React.Component<{
  ...
  render() {
    ...
    const { isPaidUser, getPro } = this.props.accountStore!;
    ...
    const isHandlerDemo =
      !isPaidUser && (this.demoHandler || isPaidHandler(rule.handler));
          ...
          <Observer>
            {() => (
              <RowContainer
                ...
                borderColor={method ? getMethodColor(method) : "transparent"}
                ...
                onKeyPress={clickOnEnter}
                depth={path.length - 1}
                style={extendRowDraggableStyles(
